...
INSERT INTO ips (name) VALUES ('걸밴크'), ('오리지널'), ('걸즈밴드크라이');
...
VALUES
  ((SELECT id FROM circles WHERE event_id=(SELECT id FROM events WHERE slug='cw-2026-07') AND slug='sample-booth'),  (SELECT id FROM events WHERE slug='cw-2026-07'), 'A-01', '18', 'https://example.com/booth', 1, '전문', '샘플 노트', 'confirmed'),
  ((SELECT id FROM circles WHERE event_id=(SELECT id FROM events WHERE slug='cw-2026-07') AND slug='sample-tsuhan'), (SELECT id FROM events WHERE slug='cw-2026-07'), NULL, NULL, NULL, 0, NULL, '통판 전용', 'unlisted');
