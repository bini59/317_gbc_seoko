...
3. Update the package version without creating an automatic npm tag: `pnpm version <version> --no-git-tag-version`.
4. Run `pnpm install --frozen-lockfile` when dependencies need installation, then run `pnpm build`.
...
7. Deploy with `pnpm deploy` only after explicit deployment confirmation. This builds the client and Worker, then runs `wrangler deploy` using `wrangler.jsonc`.
...
2. Implement the smallest safe fix and run `pnpm build` plus focused checks.
